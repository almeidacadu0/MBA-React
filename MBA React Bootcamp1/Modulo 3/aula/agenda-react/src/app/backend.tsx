...
export function getCalendarsEndpoint(): Promise<ICalendar[]> {
  return fetch("http://localhost:8080/calendars", { credentials: "include" }).then(handleResponse);
}
...
export function updateEventsEndpoint(event: IEditingEvent): Promise<IEvent[]> {
...
export function getUserEndPoint(): Promise<IUser> {
  return fetch(`http://localhost:8080/auth/user`, { credentials: "include" }).then(handleResponse);
}
